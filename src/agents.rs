...
// choose the action to take. Generic over the game so an agent written against
// the `Game` trait (like `RandomAgent`) works with any game, while a
// game-specific agent can implement it for just one. `&mut self` lets agents
// carry state between decision (an rng here; search trees or networks later).
